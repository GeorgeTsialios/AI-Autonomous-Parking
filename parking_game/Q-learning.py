...
# The player car will be controlled by an AI agent, which will use Q-learning to learn how to park the car in the parking spot. The agent will have 8 actions: move forward, move backward, turn left, turn right, move forward and turn left, move forward and turn right, move backward and turn left, move backward and turn right. 
...
class QLearningCar(AbstractCar):
    IMG = RED_CAR[0]
    ...
    def discretize_state(self):
        for radar in self.radars:
            radar[1] = radar[1] // 20        # The discretized distance has 11 bins, in range [0, 10]
