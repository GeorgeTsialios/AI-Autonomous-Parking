...
class AbstractCar:
    ...
    def calculate_START_POS(self):       
        ...
        car_spawn_index = 1 # random.randint(0, 1)
        # car_spawn = SPAWN_RECTS[car_spawn_index]
        # car_spawn_x += random.randint(0, car_spawn.width)               # randomize the spawn position of the player car
        # car_spawn_y += random.randint(0, car_spawn.height)
        car_spawn_x = 355
        car_spawn_y = 625
        # print(f"Car spawn: {car_spawn_x}, {car_spawn_y}")
        rotated_image = pygame.transform.rotate(self.img, self.angle)
        new_rect = rotated_image.get_rect(center=self.img.get_rect(topleft= (car_spawn_x, car_spawn_y)).center)
        new_rect.topleft = (car_spawn_x, car_spawn_y)
...
def train_PPO(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    model.learn(total_timesteps=steps_to_train, callback=checkpoint_callback, tb_log_name="PPO", reset_num_timesteps=False)  # Train the model
...
if __name__ == '__main__':
    ...
    # train_PPO(7000000, render=False, steps_previously_trained=3850000, run=43)
    test_PPO(1000, run=43, steps_trained=3500000, render=False)
