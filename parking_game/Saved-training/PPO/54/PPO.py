       ...
       # [radar0, radar1, radar2, radar3, radar4, radar5, radar6, radar7, offset_x, offset_y, velocity, angle]
        ...
        if terminated:
            ...
            if abs(state[8] - 0.5) >= 0.1 or abs(state[9] - 0.5) >= 0.1:    # when the car is far away from the parking spot
                ...
                if state[10] > 0.3 and state[10] < 0.5:    # punish the car for moving too slow 
...
def train_PPO(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        ...
        policy_kwargs = dict(activation_fn=th.nn.ReLU)       # change the policy network architecture to a 3-layer neural network with 128 units each
        model = PPO("MlpPolicy", env, verbose=1, tensorboard_log=log_dir, policy_kwargs=policy_kwargs, use_sde=True, sde_sample_freq=100, ent_coef=0.01, device="cpu")       # Create PPO model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
...
if __name__ == '__main__':
    ...
    # train_PPO(12000000, render=False, steps_previously_trained=0, run='54D')
    test_PPO(1000, run='54D', steps_trained=4150000, render=True)
