...
def train_PPO(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        ...
        policy_kwargs = dict(activation_fn=th.nn.Tanh, net_arch=[128, 128])       # change the policy network architecture to a 3-layer neural network with 128 units each
        model = PPO("MlpPolicy", env, verbose=1, policy_kwargs=policy_kwargs, tensorboard_log=log_dir, ent_coef=0.01, device="cpu")       # Create PPO model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
...
if __name__ == '__main__':
    ...
    train_PPO(1200000, render=False, steps_previously_trained=0, run=27)
    # test_PPO(10, run=25, steps_trained=650000, render=True)
