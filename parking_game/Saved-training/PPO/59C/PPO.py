    ...
    def step(self, action):
       ...
       # [radar0, radar1, radar2, radar3, radar4, radar5, radar6, radar7, offset_x, offset_y, velocity, angle]
        ...
        if self.current_step >= self.max_steps:
            ...
            terminated = True
            info['truncated'] = True
        elif self.num_collisions >= 2:              # Early stopping
            reward -= 100
            info["is_success"] = False
            truncated = True
            terminated = True
...
def train_PPO(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    model.learn(total_timesteps=steps_to_train, callback=checkpoint_callback, tb_log_name="PPO", reset_num_timesteps=steps_previously_trained<=1)  # Train the model
...
if __name__ == '__main__':
    ...
    # train_PPO(20000000, render=False, steps_previously_trained=1, run='59C')
    test_PPO(1000, run="59C", steps_trained=4200000, render=True)
