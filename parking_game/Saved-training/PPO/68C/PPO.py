    ...
    def step(self, action):
       ...
       # [radar0, radar1, radar2, radar3, radar4, radar5, radar6, radar7, offset_x, offset_y, velocity, angle]
        ...
        if terminated:
            ...
            self.successes += 1
            reward += 5000    
            reward += 1667 * (1 - min(abs(state[11] - 0), abs(state[11] - 1))) + 1667 * (1 - abs(state[8] - 0)) + 1667 * (1 - abs(state[9] - 0))
            
        elif inside_spot:
            reward +=  50 / (abs(state[10]) + 1)                      # reward for being inside the parking spot
            ...
            if state[10] > 0.25 or state[10] < -0.45:    # reward for moving 
                reward += 2
...
class AbstractCar:
    ...
    def check_collision(self):
        ...
        if self.collide_free_spot(new_img[1], new_img[2]):
            ...
            inside_spot = True  
            discrete_vel = round(self.vel / 6, 3) if self.vel >= 0 else round(self.vel / 4, 3)                         
            if discrete_vel == 0.0:                    # if the car is stationary in the spot
    ...
    def discretize_state(self):
        ...
        for radar in self.radars:
            radar[1] = round(2 * (radar[1] / 205) - 1, 3)
        # print(f"Radar 2: {self.radars[1][1]}", end=" ") 
        discrete_vel = round(self.vel / 6, 3) if self.vel >= 0 else round(self.vel / 4, 3)     
        # print(f"Discrete_vel: {discrete_vel}")       
        discrete_angle = round(2 * (((self.angle) % 360) / 360) - 1, 3)
        # print(f"Discrete_angle: {discrete_angle}", end=" ") 
        self.distance = round(math.sqrt(math.pow(self.center[0] - free_spot_rect.centerx, 2) + math.pow(self.center[1] - free_spot_rect.centery, 2)), 3)    # the distance of the car to the center of the parking spot
        ...
        offset_x = round((self.center[0] - free_spot_rect.centerx) / 551.65, 3)     # the offset of the car in the x direction has 2 bins, 0 if the car is to the left of the parking spot, 1 if the car is to the right of the parking spot
        offset_y = round((self.center[1] - free_spot_rect.centery) / 478.5, 3)    # the offset of the car in the y direction has 2 bins, 0 if the car is above the parking spot, 1 if the car is below the parking spot    
...
def test_PPO(test_episodes, run=1, steps_trained=0, render=True):
    ...
    for episode in range(1, test_episodes+1):
        ...
        while not terminated and not truncated:
            ...
            total_reward += reward
            print(f"A: {AgentAction(action).name} -> S: {state} R: {reward:.2f}")
...
if __name__ == '__main__':
    ...
    # train_PPO(250000000, render=False, steps_previously_trained=1, run='68C')
    test_PPO(100, run='68C', steps_trained=6000000, render=True)
