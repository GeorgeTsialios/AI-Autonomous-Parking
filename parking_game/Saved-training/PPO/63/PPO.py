    ...
    def step(self, action):
       ...
       # [radar0, radar1, radar2, radar3, radar4, radar5, radar6, radar7, offset_x, offset_y, velocity, angle]
        ...
        #         reward += 5
            ...
            reward -= 10                 # punish the car for not parking
            ...
            if abs(state[8]) < 0.05:        # reward for being close to the parking spot in the x direction
                reward += 1
                if abs(state[11]) > 0.95 or abs(state[11]) < 0.05:          # reward for being in the right angle
                    reward += 2
                    reward += 4 / (abs(state[9]) * 10 + 1)          # reward for being close to the parking spot in the y direction
...
if __name__ == '__main__':
    ...
    # train_PPO(20000000, render=False, steps_previously_trained=0, run=63)
    test_PPO(1000, run=63, steps_trained=8700000, render=True)
