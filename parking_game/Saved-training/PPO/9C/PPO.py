    ...
    def step(self, action):
       ...
       # [radar0, radar1, radar2, radar3, radar4, radar5, radar6, radar7, offset_x, offset_y, velocity, angle]
        ...
        if terminated:
            ...
            reward += self.car.difference * 5        # reward/ punishment for getting closer/ further from the center of the parking spot
        
            if inside_spot:
                reward += 10 + 10 / (abs(state[10]) + 1)                        # reward for being inside the parking spot
                ...
                if (state[11] > 0.2 and state[11] < 0.3) or (state[11] > 0.7 and state[11] < 0.8):
...
class AgentCar(AbstractCar):
    ...
    def move_player(self, agent_action):
        throttling = False   
        self.img = RED_CAR[0]           # the car image is set to the default image, so that it does not rotate when the player is not pressing the left or right arrow key     
        if agent_action == AgentAction.LEFT or agent_action == AgentAction.DOWN_LEFT or agent_action == AgentAction.UP_LEFT:                
...
if __name__ == '__main__':
    ...
    train_PPO(5000000, render=False, steps_previously_trained=3000000, run=9)
    # test_PPO(10, 9, 2900000, render=True)
