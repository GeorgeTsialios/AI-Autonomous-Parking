...
def train_PPO(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        ...
        model = PPO("MlpPolicy", env, verbose=1, tensorboard_log=log_dir, ent_coef=0.001, device="cpu")       # Create PPO model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
...
if __name__ == '__main__':
    ...
    # train_PPO(1200000, render=False, steps_previously_trained=0, run=25)
