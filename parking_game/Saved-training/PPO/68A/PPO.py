...
def test_PPO(test_episodes, run=1, steps_trained=0, render=True):
    ...
    for episode in range(1, test_episodes+1):
        ...
        while not terminated and not truncated:
            ...
            total_reward += reward
            print(f"S: {env.unwrapped.current_step:3d} A: {AgentAction(action).name:<10} -> S: {state} R: {reward:.2f}")
...
if __name__ == '__main__':
    ...
    # train_PPO(250000000, render=False, steps_previously_trained=1, run=68)
    test_PPO(100, run=68, steps_trained=2600000, render=True)
