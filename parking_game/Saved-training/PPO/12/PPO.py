...
def train_PPO(total_episodes, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        ...
        model = PPO("MlpPolicy", env, verbose=1, tensorboard_log=log_dir, ent_coef=0.01, device="cpu", learning_rate=0.0003)       # Create PPO model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
...
if __name__ == '__main__':
    ...
    # train_PPO(1000, render=False, steps_previously_trained=0, run=12)
    test_PPO(10, run=12, steps_trained=400000, render=True)
