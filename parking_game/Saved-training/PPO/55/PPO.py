       ...
       # [radar0, radar1, radar2, radar3, radar4, radar5, radar6, radar7, offset_x, offset_y, velocity, angle]
        ...
        #         reward += 5
            ...
            if abs(state[10]) < 0.25:    # punish the car for moving too slow 
                     # print("BEING STATIONARY", end=" ")
                    reward -= 2

            if abs(state[8]) <= 0.02:    # when the car is in the right x position, reward it
                reward += 1
                if  abs(state[11]) < 0.05 or abs(state[11]) > 0.95:      # when the car is in the right angle, reward it
                    reward += 1 
                    if abs(state[9]) <= 0.02:    # when the car is in the right y position
                        reward += 1 
...
if __name__ == '__main__':
    ...
    train_PPO(7000000, render=False, steps_previously_trained=0, run=55)
