...
class ParkingGameEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward += self.car.difference        # reward/ punishment for getting closer/ further from the center of the parking spot

        if terminated:
            reward += 20    
        elif state[4] == 0 and state[5] == 0:
            reward += 0.1
            if state[7] == 0:
                reward += 1
                if state[6] == 0:
...
class AbstractCar:
    ...
    def discretize_state(self):
        ...
        self.difference = 1 if previous_distance - self.distance > 0.5  else -1 if previous_distance - self.distance < -0.5 else 0    # the difference between the previous distance and the current distance has 3 bins, in range [-1, 1]
...
if __name__ == '__main__':
    ...
    # train_q(1000, render=False, episodes_previously_trained=0, checkpoint=15000)
    test_q(1000, 18000, render=True)