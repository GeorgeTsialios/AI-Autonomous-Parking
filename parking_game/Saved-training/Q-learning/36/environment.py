...
class ParkingGameEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if terminated:
            ...
            reward -= abs(state[4]) * 6
            reward -= abs(state[5]) * 6
            ...
            if state[4] == 0 and state[5] == 0:         # reward the car for being in the center of the parking spot
                reward += 5
                if state[7] == 0:                       # reward the car for having the right angle
                    reward += 20
                    # if state[6] == 0:                   # reward the car for being stationary in the center of the parking spot
                    #     reward += 1
...
def test_q(test_episodes, episodes_trained, render=True, run=0):
    ...
    q = np.load('parking_game/Saved-Training/Q-Learning/36/q_model-' + str(run) + '_' + str(episodes_trained) + '.npy')  # load Q Table from file
...
if __name__ == '__main__':
    ...
    # train_q(20000, render=False, episodes_previously_trained=0, checkpoint= -10000, run='36')
    test_q(1000, 20000, render=False, run='36')