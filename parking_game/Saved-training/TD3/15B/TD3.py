       ...
       # [radar0, radar1, radar2, radar3, radar4, radar5, radar6, radar7, offset_x, offset_y, velocity, angle]
        ...
        if terminated:
            ...
            if state[10] >= 0.4:                                               # extra reward for parking moving forward
                reward += 10 / (abs(state[10] - 0.4) + 1)                      # reward for being inside the parking spot
            else:
                reward += 5 / (abs(state[10] - 0.4) + 1)
            if state[10] <= 0.42 and state[10] >= 0.38:                   # extra reward for being stationary
...
class AbstractCar:
    ...
    def check_collision(self):
        ...
        if self.collide_free_spot(new_img[1], new_img[2]):
            ...
            if discrete_vel <= 0.42 and discrete_vel >= 0.38:                    # if the car is stationary in the spot
                # print(f"Parked & stationary for {self.count} {"frame" if self.count == 1 else "frames"}", end = " ")
                if self.count < 10:                 # if the car has been stationary for less than 10 frames, increment the counter
                    self.count += 1
                else:   # else if the car has been stationary for 10 frames, stop the game
                    terminated = True
                    self.count = 1
...
def train_TD3(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    model.learn(total_timesteps=steps_to_train, callback=checkpoint_callback, tb_log_name="TD3", reset_num_timesteps=True)  # Train the model
...
if __name__ == '__main__':
    ...
    # train_TD3(10000000, render=False, steps_previously_trained=1, run="15B")
    test_TD3(5, run="15B", steps_trained=750000, render=True)
