...
register(
    id='parking-game-v0',                          
    entry_point='TD3-instant:ParkingGameEnv', # module_name:class_name
)
...
class ParkingGameEnv(gym.Env):
    ...
    def render(self, terminated=False, truncated=False):
        ...
        TD3_text = LARGE_FONT.render(f"TD3", 1, "white") 
        WIN.blit(TD3_text, (939, 12))
...
def train_TD3(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        ...
        model = TD3("MlpPolicy", env, verbose=1, tensorboard_log=log_dir, device="auto")       # Create TD3 model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
...
def test_TD3(test_episodes, run=1, steps_trained=0, render=True):
    ...
    for episode in range(84, test_episodes+1):
        terminated = False
        truncated = False
        total_reward = 0
        reward = 0
...
if __name__ == '__main__':
    ...
    test_TD3(100, run='12', steps_trained=3850000, render=True)
