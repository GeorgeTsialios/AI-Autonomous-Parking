...
def train_TD3(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        ...
        model = TD3.load(model_path, env=env, tensorboard_log=log_dir, device="cpu")  # Load the model
    else:
        ...
        model = TD3("MlpPolicy", env, verbose=1, action_noise=action_noise, tensorboard_log=log_dir, device="cpu")       # Create TD3 model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
...
if __name__ == '__main__':
    ...
    # train_TD3(7000000, render=False, steps_previously_trained=1200000, run=1)
    test_TD3(1000, run=1, steps_trained=11500000, render=True)
