...
register(
    id='parking-game-v0',                          
    entry_point='DDPG:ParkingGameEnv', # module_name:class_name
)
    ...
    def step(self, action):
       ...
       # [radar0, radar1, radar2, radar3, radar4, radar5, radar6, radar7, offset_x, offset_y, velocity, angle]
        ...
        if terminated:
            ...
            if state[10] <= 0.4:                                               # extra reward for parking moving backwards
...
def train_DDPG(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        ...
        model = DDPG("MlpPolicy", env, verbose=1, action_noise=action_noise, tensorboard_log=log_dir, device="auto")       # Create DDPG model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
    ...
    model.learn(total_timesteps=steps_to_train, callback=checkpoint_callback, tb_log_name="DDPG", reset_num_timesteps=steps_previously_trained<=1)  # Train the model
...
if __name__ == '__main__':
    ...
    # train_DDPG(27000000, render=False, steps_previously_trained=1, run='5B')
    test_DDPG(100, run='5B', steps_trained=4700000, render=False)
