...
class ParkingGameEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward = 0
        reward -= 0.1                              # passive punishment for each step
        reward += self.car.difference * 0.2        # reward/ punishment for getting closer/ further from the center of the parking spot
        ...
        if state[4] == 0 and state[5] == 0:
            ...
            if state[7] == 0:
                ...
                if state[6] == 0:
                    reward += 3
        elif state[6] == 0:    # punish the car for standing still when it has not parked
            reward -= 0.4
