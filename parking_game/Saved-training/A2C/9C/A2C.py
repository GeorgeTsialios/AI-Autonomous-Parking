...
def train_A2C(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        ...
        model = A2C("MlpPolicy", env, n_steps=4, verbose=1, tensorboard_log=log_dir, device="cpu", ent_coef=0.01)       # Create A2C model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
...
if __name__ == '__main__':
    ...
    # train_A2C(20000000, render=False, steps_previously_trained=0, run='9C')
    test_A2C(100, run='9C', steps_trained=4500000, render=True)
