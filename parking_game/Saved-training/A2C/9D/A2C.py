    ...
    def step(self, action):
       ...
       # [radar0, radar1, radar2, radar3, radar4, radar5, radar6, radar7, offset_x, offset_y, velocity, angle]
        ...
        #         reward += 5
            ...
            if abs(state[10]) > 0.5:    # reward for moving 
                reward += 2
...
def train_A2C(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        ...
        model = A2C("MlpPolicy", env, n_steps=4, verbose=1, tensorboard_log=log_dir, device="cpu", ent_coef=0.01, use_rms_prop=False)       # Create A2C model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
...
if __name__ == '__main__':
    ...
    # train_A2C(20000000, render=False, steps_previously_trained=0, run='9D')
    test_A2C(100, run='9D', steps_trained=2500000, render=True)
