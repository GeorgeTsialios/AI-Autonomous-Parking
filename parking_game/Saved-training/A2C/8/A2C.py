...
def train_A2C(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        ...
        model = A2C("MlpPolicy", env, verbose=1, tensorboard_log=log_dir, device="cpu")       # Create A2C model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
...
if __name__ == '__main__':
    ...
    # train_A2C(7000000, render=False, steps_previously_trained=0, run=8)
    test_A2C(1000, run=8, steps_trained=6100000, render=False)
