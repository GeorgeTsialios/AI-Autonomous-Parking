    ...
    def step(self, action):
       ...
       # [radar0, radar1, radar2, radar3, radar4, radar5, radar6, radar7, offset_x, offset_y, velocity, angle]
        ...
        if terminated:
            ...
            self.successes += 1
            reward += 5000
            reward += 1000 * (1-abs(state[8])) + 1000 * (1-abs(state[9])) + 1000 * (1- min(abs(state[11]), abs(1 - state[11])))     # extra reward for being in the center of the parking spot and in the right angle  

        # elif inside_spot:
        #     if state[10] >= 0:                                               # extra reward for parking moving forward
        #         reward += 10 / (abs(state[10]) + 1)                      # reward for being inside the parking spot
        #     else:
        #         reward += 5 / (abs(state[10]) + 1)
        ...
        #         reward += 10
            ...
            if abs(state[8]) >= 0.1 or abs(state[9]) >= 0.1:    # when the car is far away from the parking spot
                if abs(state[10]) <= 0.15:    # punish the car for moving too slow
                     # print("BEING STATIONARY FAR AWAY PARKING SPOT", end=" ")
                    reward -= 5
                    if state[10] == 0:
                        reward -= 5
            else:                     # when the car is near the parking spot
                if abs(state[10]) <= 0.05:    # punish the car for moving too slow
                     # print("BEING STATIONARY NEAR PARKING SPOT", end=" ")
                    reward -= 3
                    if state[10] == 0:
                        reward -= 5
                if abs(state[11] >= 0.98) or abs(state[11] <= 0.02):      # reward the car for being in the right angle
                    reward += 0.5
...
def train_A2C(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        ...
        model = A2C("MlpPolicy", env, n_steps=4, verbose=1, tensorboard_log=log_dir, device="cpu")       # Create A2C model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
...
if __name__ == '__main__':
    ...
    train_A2C(20000000, render=False, steps_previously_trained=0, run=11)
    # test_A2C(100, run=11, steps_trained=3000000, render=True)