...
ORANGE_CAR = [scale_image(pygame.image.load("parking_game/imgs/car-orange-wheels.png"), 40/161), scale_image(pygame.image.load("parking_game/imgs/car-orange-wheels-right.png"), 40/161)]            # factor is equal to desired width of car / actual width of image
...
class ParkingGameEnv(gym.Env):
    ...
    def render(self, terminated=False, truncated=False):
        ...
        SAC_text = LARGE_FONT.render(f"SAC", 1, "white") 
        WIN.blit(SAC_text, (939, 12))
...
class PlayerCar(AbstractCar):           # the player car will have additional methods for moving using the arrow keys
    ...
    def move_player(self):
        ...
        throttling = False   
        self.img = ORANGE_CAR[0]           # the car image is set to the default image, so that it does not rotate when the player is not pressing the left or right arrow key     
        if not (keys[pygame.K_LEFT] and keys[pygame.K_RIGHT]): 
            if keys[pygame.K_LEFT]:                 # Keyboard ghosting is a hardware issue where certain combinations of keys cannot be detected simultaneously due to the design of the keyboard.
                    ...
                    self.img = pygame.transform.flip(ORANGE_CAR[1], True, False)     # and then presses the up arrow key, the car will not move, as this third key press will not be detected.                                
                    self.player_action = "LEFT "
            elif keys[pygame.K_RIGHT]:                
                    self.rotate(right=True)
                    self.img = ORANGE_CAR[1]   
...
class AgentCar(AbstractCar):
    ...
    def move_player(self, agent_action):
        ...
        if agent_action[0] < - 0.333:                 # Move Left
                ...
                self.img = pygame.transform.flip(ORANGE_CAR[1], True, False)    
        elif agent_action[0] > 0.333:                  # Move Right 
                self.rotate(right=True)
                self.img = ORANGE_CAR[1]                                # we change the car img to the one that the wheels are turning
...
def train_SAC(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    model.learn(total_timesteps=steps_to_train, callback=checkpoint_callback, tb_log_name="SAC", reset_num_timesteps=True)  # Train the model
