    ...
    def step(self, action):
       ...
       # [radar0, radar1, radar2, radar3, radar4, radar5, radar6, radar7, offset_x, offset_y, velocity, angle]
        ...
        if terminated:
            ...
            reward += 2 + 2 / (abs(state[10] - 0.5) + 1)                      # reward for being inside the parking spot
            if state[10] == 0.5:                   # extra reward for being stationary
            ...
            if abs(state[8] - 0.5) >= 0.1 or abs(state[9] - 0.5) >= 0.1:    # when the car is far away from the parking spot
                if state[10] > 0.35 and state[10] < 0.65:    # punish the car for moving too slow 
                     # print("BEING STATIONARY FAR AWAY PARKING SPOT", end=" ")
                    reward -= 2
            else:                     # when the car is near the parking spot
                if state[10] > 0.4 and state[10] < 0.6:    # punish the car for moving too slow 
...
class AbstractCar:
    ...
    def discretize_state(self):
        ...
        discrete_vel = round((self.vel + 4) / 8, 3)     
...
def train_SAC(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        ...
        model = SAC.load(model_path, env=env, tensorboard_log=log_dir, device="cpu")  # Load the model
    else:
        ...
        model = SAC("MlpPolicy", env, verbose=1, tensorboard_log=log_dir, device="cpu")       # Create SAC model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
...
if __name__ == '__main__':
    ...
    # train_SAC(7000000, render=False, steps_previously_trained=200000, run=2)
    test_SAC(1000, run=2, steps_trained=2500000, render=True)
