...
register(
    id='parking-game-v0',                          
    entry_point='SAC:ParkingGameEnv', # module_name:class_name
)
    ...
    def step(self, action):
       ...
       # [radar0, radar1, radar2, radar3, radar4, radar5, radar6, radar7, offset_x, offset_y, velocity, angle]
        ...
        if terminated:
            ...
            reward += 1667 * (1 - min(abs(state[11] - 0.5), abs(state[11] - 1), abs(state[11] - 0))) + 1667 * (1 - abs(state[9] - 0.5)) + 1667 * (1 - abs(state[8] - 0.5))          # reward for being in the right angle and in the center of the parking spot
...
models_dir = "parking_game/SAC-models"
log_dir = "parking_game/SAC-logs"
...
# Train using SAC algorithm (either from scratch or continue training)
def train_SAC(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        model_path = f"{models_dir}/sac_model-{run}_{steps_previously_trained}_steps.zip"
        model = SAC.load(model_path, env=env, tensorboard_log=log_dir, device="auto")  # Load the model
    else:
        ...
        model = SAC("MlpPolicy", env, verbose=1, train_freq=4, tensorboard_log=log_dir, device="auto")       # Create SAC model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
    ...
    model.learn(total_timesteps=steps_to_train, callback=checkpoint_callback, tb_log_name="SAC", reset_num_timesteps=steps_previously_trained<=1)  # Train the model
...
if __name__ == '__main__':
    ...
    # train_SAC(27000000, render=False, steps_previously_trained=1, run='12B')
    test_SAC(100, run='12B', steps_trained=1000000, render=True)
