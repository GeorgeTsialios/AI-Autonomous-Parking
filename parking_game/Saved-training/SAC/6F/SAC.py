    ...
    def step(self, action):
       ...
       # [radar0, radar1, radar2, radar3, radar4, radar5, radar6, radar7, offset_x, offset_y, velocity, angle]
        ...
        if terminated:
            ...
            self.successes += 1
            reward += 5000
            reward += 1000 * (1 - min(abs(state[11] - 0.5), abs(state[11] - 1), abs(state[11] - 0))) + 1000 * (1 - abs(state[9] - 0.5)) + 1000 * (1 - abs(state[8] - 0.5))          # reward for being in the right angle and in the center of the parking spot
...
def train_SAC(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    # env = SkipEnv(env, skip=2)  # Skip 2 frames per step to speed-up training
...
if __name__ == '__main__':
    ...
    # train_SAC(7000000, render=False, steps_previously_trained=0, run='6D')
    test_SAC(100, run='6D', steps_trained=200000, render=True)
