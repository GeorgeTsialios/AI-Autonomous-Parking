...
class ParkingGameEnv(gym.Env):
    ...
    def initialize_game(car_spawn_index):
        ...
        free_spot_index = random.randint(1, 10)    # the free spot will be on the same side of the player car
...
def train_SAC(steps_to_train, render=False, steps_previously_trained=0, run=1):
    ...
    if steps_previously_trained > 0:
        ...
        model = SAC.load(model_path, env=env, tensorboard_log=log_dir, device="cuda")  # Load the model
    else:
        ...
        model = SAC("MlpPolicy", env, verbose=1, tensorboard_log=log_dir, device="cuda")       # Create SAC model, MlpPolicy is a neural network with 2 hidden layers of 64 units each, it is chosen because our input is a vector of 8 values and not an image
...
if __name__ == '__main__':
    ...
    # train_SAC(20000000, render=False, steps_previously_trained=0, run='6C')
    test_SAC(1000, run='6C', steps_trained=500000, render=True)
